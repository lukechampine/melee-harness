...
bool loadPEFromSource(wibo::Executable &executable, const PeInputView &source, bool exec, bool forceRelocate) {
	...
	uintptr_t preferredBaseAddress = static_cast<uintptr_t>(header32.imageBase);
	void *preferredBase = reinterpret_cast<void *>(preferredBaseAddress);
	void *allocatedBase = nullptr;
	std::size_t allocationSize = 0;
	auto allocateImage = [&](void *base) {
		allocatedBase = base;
		allocationSize = static_cast<std::size_t>(header32.sizeOfImage);
		return wibo::heap::virtualAlloc(&allocatedBase, &allocationSize, MEM_RESERVE | MEM_COMMIT, initialProtect,
										MEM_IMAGE);
	};
	wibo::heap::VmStatus allocStatus = wibo::heap::VmStatus::InvalidAddress;
	if (forceRelocate) {
		if (preferredBaseAddress >= 0x200000) {
			allocStatus =
				allocateImage(reinterpret_cast<void *>(alignDown(preferredBaseAddress - 0x200000, 0x10000)));
			if (allocStatus != wibo::heap::VmStatus::Success) {
				DEBUG_LOG("loadPE: alternate base allocation failed (status=%u), retrying anywhere\n",
						  static_cast<unsigned>(allocStatus));
			}
		}
		if (allocStatus != wibo::heap::VmStatus::Success) {
			allocStatus = allocateImage(nullptr);
		}
	} else {
		allocStatus = allocateImage(preferredBase);
		if (allocStatus != wibo::heap::VmStatus::Success) {
 ...
 * @param forceRelocate Whether to skip the preferred image base.
 */
bool wibo::Executable::loadPE(FILE *file, bool exec, bool forceRelocate) {
	if (!file) {
		kernel32::setLastError(ERROR_BAD_EXE_FORMAT);
		return false;
	}
	return loadPEFromSource(*this, PeInputView(file), exec, forceRelocate);
}

bool wibo::Executable::loadPE(std::span<const uint8_t> image, bool exec, bool forceRelocate) {
	if (image.empty()) {
		kernel32::setLastError(ERROR_BAD_EXE_FORMAT);
		return false;
	}
	return loadPEFromSource(*this, PeInputView(image), exec, forceRelocate);
}
