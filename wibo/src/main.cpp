...
int main(int argc, char **argv) {
	...
	std::string imageName = resolvedGuestPath.filename().string();
	std::transform(imageName.begin(), imageName.end(), imageName.begin(),
				   [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
	const bool forceRelocateMain = imageName == "sjiswrap.exe";
	if (forceRelocateMain) {
		DEBUG_LOG("Relocating sjiswrap.exe away from its preferred image base\n");
	}
	if (!executable->loadPE(f, true, forceRelocateMain)) {
		fclose(f);
