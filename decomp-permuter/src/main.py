...
def post_score(
    ...
    if permuter.should_output(result):
        ...
        # Recompute the objdiff percent only on heuristic improvements (rare
        # event), and only when we actually display it (non-quiet mode).
        # Monotonically non-decreasing: if the new heuristic best regresses
        # the percent, keep the previous high.
        if (
            not context.options.quiet
            and score_value < former_best
            and result.source is not None
            and permuter.fn_name is not None
        ):
            o_file = permuter.compiler.compile(result.source)
            if o_file:
                try:
                    pct = permuter.scorer.compute_percent(o_file, permuter.fn_name)
                    if pct is not None and (
                        permuter.best_percent is None
                        or pct > permuter.best_percent
                    ):
                        permuter.best_percent = pct
                finally:
                    try_remove(o_file)
    if not context.options.quiet:
        if permuter.best_percent is not None:
            best_disp = f"{permuter.best_percent:.2f}%"
            if permuter.baseline_percent is not None:
                delta = permuter.best_percent - permuter.baseline_percent
                best_disp += f" ({delta:+.2f}%)"
        else:
            best_disp = str(permuter.best_score)
        status_line = f"iteration {context.iteration}, best = {best_disp}"
...
def run(options: Options) -> List[int]:
    ...
    try:
        ...
        if time.time() - last_time > 5:
            print("Aborting stuck process.", file=sys.stderr)
            raise
        sys.exit(0)
...
def run_inner(options: Options, heartbeat: Callable[[], None]) -> List[int]:
    ...
    for d in options.directories:
        ...
        # Optional list of functions to randomize each iteration. If empty,
        # the match target (fn_name) is randomized as usual. If non-empty,
        # the randomizer picks uniformly from this list each iteration; the
        # match target is only randomized if it appears in the list. This
        # lets you ask the permuter to focus solely on a helper, e.g. a
        # static inline used by the match target.
        permute_fn_names: Tuple[str, ...] = ()
        if "permute_fn_names" in settings:
            permute_fn_names = tuple(
                json_array(settings["permute_fn_names"], str)
            )
        else:
            try:
                with open(
                    os.path.join(d, "permute_fns.txt"), encoding="utf-8"
                ) as f:
                    permute_fn_names = tuple(
        ...
        try:
            permuter = Permuter(
                ...
                debug_mode=options.debug_mode,
                speed=options.speed,
                permute_fn_names=permute_fn_names,
            )
    ...
    for permuter in context.permuters:
        ...
        # Seed display-only percent tracking from the wrapper's baseline.
        permuter.baseline_percent = options.baseline_percent
        permuter.best_percent = options.baseline_percent
...
def main() -> None:
    ...
    parser.add_argument(
        "--baseline-percent",
        dest="baseline_percent",
        type=float,
        default=None,
        help="objdiff-cli percentage of the unmodified base. The live counter "
        "shows the running best as 'X.XX%% (+Y.YY%%)' relative to this. The "
        "wrapper script computes it once at startup; not normally set by hand.",
    )
