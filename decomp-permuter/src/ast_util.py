...
import re
from typing import Callable, Dict, Iterable, List, Optional, Set, Tuple, TYPE_CHECKING, Union
...
def extract_fn(
    ast: ca.FileAST,
    fn_name: str,
    preserve_fn_names: Iterable[str] = (),
) -> Tuple[ca.FuncDef, int]:
    """Find ``fn_name`` in the file-level AST and return its FuncDef + index.

    As a side effect, every other non-``inline`` FuncDef in the file gets
    rewritten in place to its declaration, so the permuter can compile the
    file with only the target function's body present. Pass any additional
    function names whose bodies must be kept intact (e.g. extra helpers that
    the permuter will also randomize) via ``preserve_fn_names``.
    """
    keep = {fn_name, *preserve_fn_names}
    ret = []
    for i, node in enumerate(ast.ext):
        if isinstance(node, ca.FuncDef):
            if node.decl.name == fn_name:
                ret.append((node, i))
            elif node.decl.name in keep:
                # Another randomization target — leave its body intact.
                pass
