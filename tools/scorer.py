"""
Disassembly-based candidate scorer, vendored from decomp-permuter (MIT):
https://github.com/simonlindholm/decomp-permuter (src/scorer.py). permute.py
uses it to score a candidate object against the target. The only change from
upstream is the objdump import below (sibling module, not package-relative).
"""
...
class Scorer:
    ...
    def score(self, cand_o: Optional[str]) -> Tuple[int, str]:
            ...
            for nf, of in zip(newfields, oldfields):
                if nf != of:
                    ...
                    if field_matches_any_symbol(nf, self.arch) and old_line.has_symbol:
                        continue
                    # If both lines have relocations, ignore symbol name
                    # differences (equivalent to objdiff data_value mode)
                    if old_line.has_symbol and new_line.has_symbol:
                        continue
        ...
        def diff_insert(line: Line) -> None:
            # Reordering or totally different codegen.
            # Defer this until later when we can tell.
            if not line.has_symbol:
                insertions.append(line.row)

        def diff_delete(line: Line) -> None:
            if not line.has_symbol:
                deletions.append(line.row)
        ...
        for tag, i1, i2, j1, j2 in result_diff:
            ...
            if tag == "replace" or tag == "delete":
                for k in range(i1, i2):
                    diff_insert(cand_seq[k])
            if tag == "replace" or tag == "insert":
                for k in range(j1, j2):
                    diff_delete(self.target_seq[k])
    ...
    def compute_percent(self, cand_o: str, fn_name: str) -> Optional[float]:
        """Run objdiff-cli on cand_o vs target_o, returning the match
        percentage. Returns None on failure. Used only for display."""
        try:
